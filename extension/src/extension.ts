 ...
 */
function convertDiagnostic(vsDiagnostic: vscode.Diagnostic): Diagnostic {
  try {
    return {
      range: {
        start: {
...
function mapDiagnosticSeverity(
  ...
  try {
    ...
    log(
      `ERROR in mapDiagnosticSeverity: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
    return DiagnosticSeverity.Information;
 ...
 */
function mapSymbolKind(kind: vscode.SymbolKind | undefined): SymbolKind {
  try {
    switch (kind) {
      ...
      default:
        return SymbolKind.Variable; // Default fallback
    }
  } catch (error) {
    log(
      `ERROR in mapSymbolKind: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
    return SymbolKind.Variable; // Default fallback
...
class LanguageServerConnection {
  ...
  private diagnosticSubscriptions: vscode.Disposable[] = [];
  ...
  private setupConnectionHandlers(): void {
    ...
    this.connection.onRequest(
      ...
      this.handleDiagnosticRequest.bind(this)
    );

    // Handle workspace symbol request
    this.connection.onRequest(
      "workspace/symbol",
      this.handleWorkspaceSymbolRequest.bind(this)
    );

    // Handle document symbol request
    this.connection.onRequest(
      "textDocument/documentSymbol",
      this.handleDocumentSymbolRequest.bind(this)
    );

    // Handle implementation request
    this.connection.onRequest(
      "textDocument/implementation",
      this.handleImplementationRequest.bind(this)
    );
  ...
  private async handleDidOpenTextDocument(
    ...
    try {
      ...
      log(
        `ERROR in handleDidOpenTextDocument: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
   ...
   */
  private setupDiagnosticChangeSubscription(
  ...
  private async handleDiagnosticRequest(params: any): Promise<any> {
    try {
      log(
        `textDocument/diagnostic request received for ${params.textDocument.uri}`
      );
      ...
      log(
        `Found ${diagnostics.length} diagnostics for ${params.textDocument.uri}`
      );
      ...
      log(
        `Returning ${items.length} diagnostic items for ${params.textDocument.uri}`
      );
      return { kind: "full", items };
    } catch (error) {
      log(
        `ERROR in handleDiagnosticRequest: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      if (error instanceof Error && error.stack) {
   ...
   * Handle workspace symbol request
   */
  private async handleWorkspaceSymbolRequest(params: any): Promise<any> {
    try {
      ...
      log(
        `ERROR in handleWorkspaceSymbolRequest: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      if (error instanceof Error && error.stack) {
   ...
   * Handle document symbol request
   */
  private async handleDocumentSymbolRequest(params: any): Promise<any> {
    try {
      ...
      log(
        `ERROR in handleDocumentSymbolRequest: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      if (error instanceof Error && error.stack) {
   ...
   * Handle implementation request
   */
  private async handleImplementationRequest(params: any): Promise<any> {
    try {
      ...
      log(
        `ERROR in handleImplementationRequest: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      if (error instanceof Error && error.stack) {
   ...
   */
  private convertDocumentSymbol(symbol: vscode.DocumentSymbol): any {
    return {
      name: symbol.name,
      kind: mapSymbolKind(symbol.kind),
      range: {
 ...
 */
async function startLanguageServer(): Promise<net.Server> {
  // Create connections set to track active connections
  const connections: Set<LanguageServerConnection> = new Set();
  ...
  const server = net.createServer((socket) => {
    ...
    try {
      ...
      socket.on("close", () => {
        ...
        log(
          `Connection removed from tracking, ${connections.size} active connections remaining`
        );
      });
    } catch (error) {
      log(
        `ERROR in server connection handler: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      if (error instanceof Error && error.stack) {
